              ...
              <v-card-title>
                <v-row class="pl-4 pr-4">
                  <v-col cols="12" sm="4" md="4">
                    ...
                    <v-text-field
                      class="classicInput"
                      solo v-model="name" dense
                      :rules="nameRules" label="Nombre">
                    </v-text-field>
                    ...
                    <div v-if="instructions.length > 0">
                      <div class="my-3" v-for="(items4, index4) of instructions" :key="'G' + index4">
                        <v-card class="elevation-0" width="100%" style="z-index: 100 !important">
                          <v-icon class="btnFieldText" color="red"
                            @click="removeInstructionsOfArray(index4, items4.id)">
                            mdi-close-circle
                          </v-icon>
                        </v-card>
                        <v-text-field
                          v-model="instructions[index4].title"
                          dense solo
                  ...
                  <v-col cols="12" sm="4" md="4">
                    ...
                    <div v-if="officeItems.length > 0">
                          ...
                          <v-card-text class="pt-6">
                            ...
                            <!-- <div v-if="iconsTemp.length == 0">
                              <v-card
                                ...
                                :key="index" width="50px" height="50px"
                            ...
                            <div v-if="iconsTemp.length == 0">
                              <v-card
                                ...
                                height="50px"
                                @click="selectIcon(icon, indexIcons)">
                                <img :src="icon.svg | DeleteUrl" class="imgIconOfficeItem"></img>
                              </v-card>
                            </div>
                      ...
                      <!-- <div outlined class="my-3" v-for="(items3, index3) of officeItems" :key="index3">
                        <div v-if="items3.status == 'active'">
                          <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                              <v-btn elevation="1"
                                @click="openDialog(index3)" width="30px" height="30px"
                                color="primary" class="float-left mt-1 ml-2"
                                fab small v-bind="attrs" v-on="on">
                                <v-icon size="15px">mdi-{{ items3.image }}</v-icon>
                              </v-btn>
                            </template>
                            <span>Seleccionar icono</span>
                          </v-tooltip>

                          <v-card-title class="py-0 text-body-1">
                            <v-card
                              class="elevation-0" width="100%"
                              style="z-index: 100 !important">
                              <v-icon class="btnFieldText" color="red"
                                @click="removeItemsOfArray(index3, items3.id)">
                                mdi-close-circle
                              </v-icon>
                            </v-card>
                            <v-text-field class="" v-model="items3.name" dense solo placeholder="icono">
                            </v-text-field>
                          </v-card-title>
                        </div>
                      </div> -->
                      <v-card outlined class="my-3"
                        ...
                        <v-row>
                          <v-col class="colImgOfficeItems">
                            <v-tooltip bottom>
                              <template v-slot:activator="{ on, attrs }">
                                ...
                                :src="items3.image.svg | DeleteUrl">
                                </img>
                              </template>
                              <span>Seleccionar icono</span>
                            </v-tooltip>
                          ...
                          <v-col class="colTextOfficeItems">
                            ...
                            {{ items3.name }}
                            <v-icon class="btnFieldText" color="red" @click="removeItemsOfArray(index3, items3.id)">
                              mdi-close-circle
                            </v-icon>
                          </v-card-title>
                          </v-col>
                    ...
                    <v-text-field
                      class="classicInput"
                      solo v-model="capacity" min="0" dense
                      type="number" :rules="[rules.required]" label="Capacidad">
                    </v-text-field>
                  </v-col>
                  <v-col cols="12" sm="4" md="4">
                    ...
                    <input type="file"
                      ...
                      id="inputForUploadImage" accept="image/jpg"
                      size="1" multiple @change="onFilePicked"/>

                    <v-card outlined class="pb-5" v-if="images.length == 0">
                      <v-card-title class="d-flex justify-center">
                        <v-icon size="60" @click="openImage" color="primary">mdi-plus-circle</v-icon>
                      </v-card-title>
                    ...
                    <v-card elevation="0" v-for="(image4 , inx) of images " :key="'A' + inx" v-else>
                      <div
                        class="img-checks"
                        @click="openImage"
                        :style="'background-image:url(' + image4.replace('https://www.kpmgexternalservices.com.co:5001','https://boapionekey.co.kworld.kpmg.com:5001') + ')'"
                      ></div>
                    ...
                    <input type="file"
                      style="display: none" ref="image2"
                      id="inputForUploadImage2" accept="image/jpg"
                      size="1" multiple @change="onFilePicked2"/>

                    <v-card outlined class="pb-5" v-if="images2.length == 0">
                      <v-card-title class="d-flex justify-center">
                        <v-icon size="60" @click="openImage2" color="primary">mdi-plus-circle</v-icon>
                      </v-card-title>
                      <v-card-text class="px-16 text-center">
                        <h2 style="color: #c7c7c7 !important;">Agregar Fotos</h2>
                      </v-card-text>
                    ...
                    <v-card elevation="0" v-for="(image2, index2) of images2" :key="index2" v-else>
                      <div class="img-checks" @click="openImage2"
                        :style="'background-image:url(' + image2.replace('https://www.kpmgexternalservices.com.co:5001','https://boapionekey.co.kworld.kpmg.com:5001') + ')'">
                      </div>
                    ...
                    <!-- <span class="float-left primary--text"
                      style="cursor: pointer !important" @click="generateQrCode">
                      Generar código qr
                    </span>
                    <div class="d-flex justify-end primary--text" @click="downloadQrCode">
      ...
      <v-row class="pl-4 pr-4">
        <v-col class="mx-auto" cols="12" sm="11" md="11">
          <v-btn
            ...
            class="text-capitalize px-10" elevation="3" outlined
            color="red" @click="$router.push('/cities')">
            Cancelar
          </v-btn>
          <v-btn
            style="background-color: white !important; border-radius: 8px"
            class="text-capitalize px-14 ml-10" @click="updateOffice"
            elevation="3" outlined color="#0691DA">
            Editar
          </v-btn>
        </v-col>
      </v-row>
...
export default {
    ...
    return {
      itemBreadcrumbs: [
        ...
        },
        {
          text: 'Editar Espacio',
          disabled: true,
          href: '/cities/update-space',
        },
      ],
      disabledBuilding: false,
      disabledFloor: false,
  ...
  mounted() {
    ...
    // this.listBuildings();
    this.listOfficeType();
    this.officeArrivalsDirectionList();
  ...
  methods: {
    ...
    addInatructionsToArray() {
      ...
      if (this.instructionsText != "" && emptyField.test(this.instructionsText)) {
        ...
        this.instructions.push({
            id: 0,
            title: this.instructionsText,
          });
        this.instructionsText = "";
      }
    },
    async removeInstructionsOfArray(index, id) {
      if(id !== 0){
        let formTemp = {
          acstkn: localStorage.getItem("acstkn"),
          id: id,
        };
        const response = await this.$store.dispatch("office/statusInstruction", formTemp);
        if(response.success){
    ...
    async removeItemsOfArray(index, id) {
      ...
      if(id !== 0){
        ...
        const response = await this.$store.dispatch("office/statusItem", formTemp);
    ...
    async officeArrivalsDirectionList() {
      let formTemp = {
        acstkn: localStorage.getItem("acstkn"),
        limit: 30,
        offset: 0,
      ...
      const response = await this.$store.dispatch(
        "officeArrivalsDirection/officeArrivalsDirectionList",
        formTemp
      );
      this.instructions = [];
      if (response.success) {
    ...
    async listOffice() {
      ...
      const response = await this.$store.dispatch("office/officeOne", formTemp);
      if (response.success) {
        // console.log(response.body)
        this.name = response.body.name;
        this.description = response.body.description;
        this.capacity = response.body.maxCapacity;
        this.city = response.body.cityId;
        this.cityToGetBuildings = response.body.cityId;
        this.building = response.body.buildingId;
        this.listFloorBuilding();
        this.officeType = response.body.officeTypeId;
        this.floorBuilding = response.body.floorBuildingId;
        this.checkIn = response.body.durationCheckIn;
        this.checkOut = response.body.durationCheckOut;
        this.qrCode = response.body.qrCode;
        // this.qrCode = "2";
        if(response.body.officePlains.length > 0){
          this.images.push(response.body.officePlains[0].image);
        }
        if (response.body.officeImages.length >0) {
          this.images2.push(response.body.officeImages[0].image);
        }

        response.body.officeItems.map((respon) => {
          if(respon.status === 'active'){
            this.officeItems.push({
              id: respon.id,
              name: respon.name,
              image: respon.systemItemIcons,
    ...
    async updateOffice() {
      if (this.$refs.formSpaceUpdate.validate()) {
        if (
          this.description !== "" &&
          this.name !== "" &&
          this.building !== "" &&
  ...
  computed: {},
  filters: {
    DeleteUrl: function (value) {
      //return value;
      return value.replace('http://kpmgapi.inkubo.co:5005','http://kpmgapi.inkubo.co:5005');
      //return value.replace('https://www.kpmgexternalservices.com.co:5001','https://boapionekey.co.kworld.kpmg.com:5001');
    }
  },
};
</script>

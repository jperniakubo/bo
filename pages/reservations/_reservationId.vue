              ...
              <v-card height="100%">
                <v-card-text class="black--text">
                  <v-avatar
                    ...
                    "
                  >
                    <img :src="reservation.users.profileImage | DeleteUrl" />
                  </v-avatar>
                  <h2>{{ reservation.users.fullName }}</h2>
                  <h2 class="text-body-1 primary--text">
                    {{ reservation.users.position }}
                  </h2>
                  ...
                  >
                    <v-img
                      ...
                      width="100%"
                      :src="
                        reservation.office.officeImages[0].image.replace(
                          'https://www.kpmgexternalservices.com.co:5000',
                          'https://boapionekey.co.kworld.kpmg.com:5000'
                        )
                      "
                    >
                      ...
                      <v-row justify="space-between">
                        ...
                        <v-col cols="12" sm="10" md="10" class="text-left">
                          ...
                          <p class="text-body-2 ma-0">
                            {{ reservation.office.building.name }}
                          </p>
                        </v-col>
                      </v-row>
                    </v-card-text>
                  ...
                  >
                    ...
                    <v-card-text class="text-left black--text pt-0">
                      ...
                      <p class="text-body-1 ma-0">
                        Reservado por: {{ reservation.users.fullName }}
                      </p>
                      <p class="text-body-1 ma-0">
                        Número Personas en sala:
                        {{ reservation.office.maxCapacity }}
                      </p>
...
export default {
  ...
  async asyncData({ store, params }) {
    ...
    if (response.success) {
      let timeStart =
        response.body.date + "T" + response.body.startTime + ":00-05:00";
      let timeEnd =
        response.body.date + "T" + response.body.endTime + ":00-05:00";

      response.body.date = moment(response.body.date)
        .format("MMMM D, YYYY")
        .toUpperCase();
  ...
  filters: {
    DeleteUrl: function(value) {
      ...
      return value.replace(
        "http://kpmgboapi.inkubo.co:50002",
        "https://boapionekey.co.kworld.kpmg.com:5001"
      );
      //return value.replace('https://www.kpmgexternalservices.com.co:5000','https://boapionekey.co.kworld.kpmg.com:5001');
    }
  }
};
</script>

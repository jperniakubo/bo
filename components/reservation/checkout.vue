<template>
  <v-col cols="12" sm="4" md="4">
    <v-card class="px-3 pt-0" height="100%" v-if="options.checkOut.length != 0">
      <v-card-text class="text-left">
        <h2 class="black--text text-h4 font-weight-medium">Check Out</h2>
        ...
        <div>
          <v-row v-for="(items, index) of options.checkOut[0].OfficeItems" :key="index">
            ...
            <v-col cols="12" sm="2" md="2" class="pa-0">
              <v-icon class="pa-0 ma-0" color="#005EB8" v-if="items.ItemsCheckOut.checked == 1"
        ...
        <v-row style="height:250px !important; overflow: auto;">
          ...
          <v-col cols="12" sm="6" md="6"   v-for="(image, index) of options.checkOut[0].imagesCheckOut" :key="index">
            <div
    ...
    <v-card class="px-3 pt-0 d-flex align-center" height="100%" v-else>
      <v-card-text>
        ...
        <p class="grey--text sfProDisplayRegular txtEmptyCheckInCheckOut">
          En este momento no sea realizado check out.
        </p>
      </v-card-text>
    </v-card>
  </v-col>
